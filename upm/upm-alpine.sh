...
# implement "upm" by wrapping apk commands
...
update)
	...
	#todo: update upm-alpine codev-util codev-shell codev (in /usr-new/local)
	
	sh /usr/local/share/upm/upm-bootup.sh
	mv /usr-new /usr
	rm /tmp/upm-status
	;;
autoupdate)
	...
	upm update || echo error > /tmp/upm-status
	;;
install)
	shift
	package=
